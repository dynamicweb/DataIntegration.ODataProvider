...
namespace Dynamicweb.DataIntegration.Providers.ODataProvider
{
    ...
    [AddInLabel("BC Provider")]
    [AddInDescription("Business central provider")]
    [AddInIgnore(false)]
    public class BCProvider : BaseEndpointProvider, IDropDownOptionActions
    {
        public new Hashtable GetOptions(string name)
        {
            ...
            {
                foreach (var endpoint in GetEndpoints(typeof(BCProvider)))
                {
        ...
        public BCProvider() : base() { }

        public BCProvider(XmlNode xmlNode) : base(xmlNode) { }
        ...
        public override bool RunJob(Job job)
        {
            ...
            foreach (var mapping in job.Mappings)
            {
                ...
                using (var writer = new BCWriter(baseEndpointWriter))
                {
